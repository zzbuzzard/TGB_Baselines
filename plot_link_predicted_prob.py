...
def get_probabilities_by_time(
    ...
    with torch.no_grad():
        ...
        for start in tqdm(range(0, count, batch)):
            ...
            src_embeds, dst_embeds = query_pred_edge_batch(
                model_name=model_name,
                model=model,
                src_node_ids=src_ids[start:end],
                dst_node_ids=dst_ids[start:end],
            ...
            predicted_probs = (
                model[1](input_1=src_embeds, input_2=dst_embeds)
                .squeeze(dim=-1)
                .sigmoid()
            )
...
def main(save_all=False, run=0, neg_spacing=1):
    ...
    save_model_folder = (
        f"./saved_models/{args.model_name}/{args.dataset_name}/{args.save_model_name}/"
    )
    ...
    test_pairs = list(
        zip(test_data.src_node_ids.tolist(), test_data.dst_node_ids.tolist())
    )
    ...
    for src, dst in pairs:
        ...
        times, probs = get_probabilities_by_time(
            model_name=args.model_name,
            model=model,
            neighbor_sampler=full_neighbor_sampler,
        ...
        hop0, hop1, hop2 = get_temporal_edge_times(
            dataset, src - 1, dst - 1, num_hops=2, mask=dataset.test_mask
        )
        ...
        for hop_threshold in range(4):
            totvar, totvar_per_sec = total_variation_per_unit_time(
                [hop0, hop1, hop2][:hop_threshold], probs, times
            )
        ...
        if save_all:
            ...
            plt.show()
            plt.savefig(
                f"{GraphMixer}_{args.time_encoder}_{src}_{dst}.pdf",
                bbox_inches="tight",
            )
